# [PHPFusion Docs](../home.md)

# Class: \PHPFusion\Database\Exception\SelectionException
### Namespace: [\PHPFusion\Database\Exception](../namespaces/PHPFusion.Database.Exception.md)
---
**Summary:**

Exception class for unsuccessful DB selection

**Description:**

Throw it when the connection is OK but the given database
cannot be selected.

---
### Constants
* No constants found
---
### Properties
---
### Methods
---
### Details
* File: [classes\PHPFusion\Database\Exception\SelectionException.php](../files/classes.PHPFusion.Database.Exception.SelectionException.md)
* Package: Default
* Class Hierarchy: 
  * [\RuntimeException]()
  * \PHPFusion\Database\Exception\SelectionException


---

### Top Namespaces

* [\Defender](../namespaces/Defender.html.md)
* [\PHPFusion](../namespaces/PHPFusion.html.md)

---

### Reports
* [Errors - 1023](../reports/errors.md)
* [Markers - 6](../reports/markers.md)
* [Deprecated - 36](../reports/deprecated.md)

---

This document was automatically generated from source code comments on 2021-07-21 using [phpDocumentor](http://www.phpdoc.org/) and [fr3nch13/phpdoc-markdown](https://github.com/fr3nch13/phpdoc-markdown)
